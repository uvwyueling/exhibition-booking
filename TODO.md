...
**现状**：已正式上线（Vercel + `www.ysxy-exhibition.cloud`，公开可访问）。后端读写档期在 Vercel、邮件/PDF 在腾讯 SCF，两套独立运行。
**下一步优先级**：A 两块功能（前三天登记表邮件、驳回邮件）。建议从「前三天《登记表》邮件」起步 —— 表单生成已复用现成的 `formpdf.js`，只差每日定时触发 + 日期匹配，最省事。
